      ...
      <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-slate-200/90 overflow-hidden z-10 font-sans flex flex-col max-h-[80vh]">
        ...
        <form onSubmit={handleSearchSubmit} className="relative flex items-center px-5 py-4 border-b border-slate-100 bg-slate-50/50">
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-2.5 py-1 rounded-xl bg-slate-200/80 hover:bg-slate-300/80 text-[11px] font-black text-slate-600 transition border-none cursor-pointer"
          >
            ESC
          </button>
        </form>
        ...
        <div className="p-5 overflow-y-auto space-y-6">
          {trimmed ? (
            /* Search Results */
            <div>
              <div className="flex items-center justify-between mb-3 text-xs text-slate-400 font-extrabold uppercase tracking-wider">
              ...
              {searchResults.length > 0 ? (
                ...
                <div className="py-10 text-center text-slate-400">
                  ...
                  <p className="text-xs font-bold text-slate-600">No products found matching "{query}"</p>
                  <p className="text-[11px] text-slate-400 mt-1">Try searching for "Business Cards", "Banners", or "Brochures"</p>
                </div>
              )}
            </div>
          ) : (
            ...
            <div className="space-y-6">
              ...
              <div>
                <div className="flex items-center gap-1.5 text-xs text-slate-400 font-extrabold uppercase tracking-wider mb-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {popularChips.map((chip) => (
                    <button
                      ...
                      className="px-3.5 py-2 rounded-xl bg-slate-100 hover:bg-[#FF5A1F]/10 text-slate-800 hover:text-[#FF5A1F] text-xs font-bold transition border border-slate-200/80 hover:border-[#FF5A1F]/30 cursor-pointer flex items-center gap-1.5"
                    >
              ...
              {recentSearches.length > 0 && (
                <div>
                  <div className="flex items-center justify-between text-xs text-slate-400 font-extrabold uppercase tracking-wider mb-3">
                  ...
                  <div className="space-y-1.5">
                    {recentSearches.map((term, idx) => (
                      <button
                        ...
                        className="w-full px-3.5 py-2 rounded-xl hover:bg-slate-100 text-left text-xs font-bold text-slate-700 flex items-center justify-between transition border-none bg-transparent cursor-pointer"
                      >
