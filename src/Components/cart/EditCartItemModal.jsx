  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 space-y-5 text-xs text-[#0B1633]">
          <div>
          ...
          <div className="space-y-1.5">
            <label className="font-extrabold text-xs block text-slate-800">Quantity (Units):</label>
            <div className="flex items-center gap-3">
              ...
              />
              <span className="text-xs text-slate-500 font-medium">Unit Price: ₹{unitPrice}</span>
            </div>
          </div>

          {/* Artwork Files Section */}
          <div className="space-y-2">
            <label className="font-extrabold text-xs block text-slate-800">Uploaded Artwork Files ({artworkFiles.length}):</label>
            
            {artworkFiles.length > 0 && (
              <div className="space-y-1.5 max-h-40 overflow-y-auto">
                {artworkFiles.map((f, idx) => (
                  <div key={idx} className="p-2.5 rounded-xl bg-[#FAFBFD] border border-slate-200 flex items-center justify-between text-xs">
            ...
            {errorMsg && <p className="text-rose-600 font-bold text-[11px]">{errorMsg}</p>}
          </div>

          {/* Artwork Notes */}
          <div className="space-y-1">
            <label className="font-extrabold text-xs block text-slate-800">Special Instructions:</label>
            <textarea
              rows={2}
              value={artworkNotes}
              onChange={(e) => setArtworkNotes(e.target.value)}
              className="w-full bg-[#F7F8FA] border border-slate-200 rounded-xl p-2.5 text-xs text-[#0B1633] focus:outline-none focus:border-[#FF5A1F] resize-none"
            />
          </div>
          ...
          <div className="p-3.5 rounded-2xl bg-orange-50/60 border border-orange-200/60 flex items-center justify-between">
            <span className="font-bold text-slate-700 text-xs">Recalculated Item Total:</span>
        ...
        <div className="p-4 bg-[#FAFBFD] border-t border-slate-200 flex items-center justify-end gap-3">
          <button onClick={onClose} className="px-4 py-2.5 rounded-xl border border-slate-300 font-bold text-xs text-slate-700 hover:bg-slate-100 transition cursor-pointer">
            Cancel
          </button>
          <button onClick={handleSave} className="px-6 py-2.5 rounded-xl bg-[#FF5A1F] hover:bg-[#e44d15] text-white font-extrabold text-xs shadow-md transition cursor-pointer flex items-center gap-1.5 border-none">
